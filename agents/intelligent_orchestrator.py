...
from .car_agent import run_car_agent  # [prev: no car_agent import]
...
# near other imports
from .primary_intent_agent import Product  # already present
# Add this local helper below the logger:
_ALLOWED_INTENTS = {
    "greeting","product_inquiry","informational","payment_inquiry",
    "policy_claim_status","clarification_needed","invalid_input","other"
}

def _normalize_intent_result(user_message: str, ir):
    """Router-side guard in case the LLM returns odd labels."""
    text = (user_message or "").lower()
    # intent
    intent = getattr(ir, "intent", None)
    if hasattr(intent, "value"):
        intent_str = intent.value
    else:
        intent_str = str(intent or "")
    if intent_str not in _ALLOWED_INTENTS:  # [prev: no guard, 'unwanted' slipped through]
        if any(w in text for w in ["buy","purchase","i want","quote","get a plan"]):
            ir.intent = "product_inquiry"
        elif any(w in text for w in ["what","cover","benefit","policy","include","exclusion"]):
            ir.intent = "informational"
        else:
            ir.intent = "other"

    # product
    prod = getattr(ir, "product", Product.UNKNOWN)
    if isinstance(prod, str):
        try:
            ir.product = Product(prod.upper())
        except Exception:
            ir.product = Product.UNKNOWN
    if ir.product == Product.UNKNOWN:
        if any(w in text for w in ["car","auto","motor","vehicle","tpft","workshop"]):
            ir.product = Product.CAR
        elif any(w in text for w in ["maid","helper","domestic"]):
            ir.product = Product.MAID
        elif any(w in text for w in ["travel","trip","vacation","holiday"]):
            ir.product = Product.TRAVEL
    # confidence floor
    if getattr(ir, "confidence", 0) < 0.5:
        ir.confidence = 0.75
    return ir
...
def handle_unknown_product_intelligently(user_message: str, chat_history: list, session_id: str) -> str:
    ...
    try:
        ...
        if hasattr(intent_result, 'product') and intent_result.product != Product.UNKNOWN:
            if intent_result.product == Product.TRAVEL:
            ...
            elif intent_result.product == Product.CAR:  # [prev: no CAR branch here]
                ...
                car_response = run_car_agent(user_message, chat_history, session_id)
                if isinstance(car_response, dict):
                    return car_response.get("output", "I'd be happy to help with car insurance! 🚗")  # [prev: N/A]
                else:
                    return car_response  # [prev: N/A]
        ...
        if hasattr(intent_result, 'intent'):
            if intent_result.intent == "payment_inquiry":
                ...
                if collected_info:
                    ...
                    payment_response = run_payment_agent(user_message, chat_history, session_id)  # [prev: run_payment_agent(user_message, chat_history)]
...
🚗 **Car Insurance** - for your vehicle"""  # [prev: only Travel & Maid listed]
...
Which type would you like to explore? Once you choose, I'll ask a few quick questions and recommend the perfect plan with all the coverage details! ✨"""  # [prev: only Travel & Maid listed]
...
• Travel Insurance
• Maid Insurance
• Car Insurance"""  # [prev: only Travel & Maid listed]
...
def provide_intelligent_guidance(user_message: str, chat_history: list, session_id: str) -> str:
    ...
    try:
        ...
        if len(chat_history) <= 2:
            ...
            return "Great! I can help you with Travel, Maid, or Car insurance. Which would you like to learn about? ✈️🏠🚗"  # [prev: only Travel & Maid mentioned]
        else:
            # Later in conversation - provide contextual guidance
            return "I want to make sure I help you with the right information! Are you interested in:\n\n🌍 Travel insurance for your trips\n🏠 Maid insurance for your domestic helper\n🚗 Car insurance for your vehicle\n\nJust let me know which one! 😊"  # [prev: only Travel & Maid mentioned]
...
def get_whatsapp_fallback_response(intent_type: str, requires_clarification: bool = False) -> str:
    ...
    responses = {
        ...
        "off_topic": [
            "I specialize in helping with insurance questions! 😊 What can I help you with regarding Travel, Maid, or Car insurance?",
            "I'm here to assist with your insurance needs. How can I help with Travel, Maid, or Car insurance today? 🛡️",
            "Let's talk insurance! I can help you with Travel, Maid, or Car insurance. What would you like to know? ✈️🏠🚗"
        ],  # [prev: only Travel & Maid mentioned in each string]
        "error": [
...
def get_contextual_greeting() -> str:
    ...
    greetings = [
        f"{time_greeting}! 😊 I'm here to help you with Travel, Maid, and Car insurance. What can I assist you with today?",  # [prev: Travel and Maid only]
        f"{time_greeting}! 👋 Welcome to HLAS! I can help you with Travel, Maid, or Car insurance. How may I help you?",       # [prev: Travel or Maid]
        f"{time_greeting}! 🌟 I'm your insurance assistant. I specialize in Travel, Maid, and Car insurance. What would you like to know?"  # [prev: Travel and Maid only]
    ]
    ...
    try:
        ...
        if stage == "payment":
            try:
                ...
                response_data = run_payment_agent(user_message, chat_history, session_id)  # [prev: run_payment_agent(user_message, chat_history, session_id) — unchanged if already like this]
...
{chat_history[-6:] if len(chat_history) > 6 else chat_history}"""),
                ...
                if intent_result.intent == "purchase" and intent_result.confidence > 0.6:
                    ...
                    set_stage(session_id, "payment")
                    agent_response = run_payment_agent(user_message, chat_history, session_id)  # [prev: run_payment_agent(user_message, chat_history, session_id)]
...
- CAR: Car/motor/auto insurance, workshop plans, TPFT  # [prev: no CAR line here]
- UNKNOWN: Cannot determine or not specified

Conversation context: The agent just offered help with Travel, Maid, or Car insurance.  # [prev: Travel or Maid]
...
def process_normal_intent(intent_result, user_message: str, chat_history: list, session_id: str) -> str:
    ...
    try:
        ...
        agent_map = {
            ...
            Product.CAR: run_car_agent,  # [prev: no CAR mapping]
        }
        ...
        if intent == "payment_inquiry":
            ...
            set_stage(session_id, "payment")
            try:
                response_data = run_payment_agent(user_message, chat_history, session_id)  # [prev: run_payment_agent(user_message, chat_history)]
                if isinstance(response_data, dict):
            ...
            try:
                ...
                # All product agents now accept (user_message, chat_history, session_id)
                response_data = agent_function(user_message, chat_history, session_id)  # [prev: per-product conditional calls]
...
if __name__ == '__main__':
    ...
    # --- Test Case 3: Car Inquiry ---  # [prev: no Car test]
    print("--- Testing Car Inquiry ---")
    user_msg_2b = "I want to buy car insurance."
    response_2b = orchestrate_chat(user_msg_2b, session_id)
    print(f"User: {user_msg_2b}")
    print(f"Bot: {response_2b}")
    print("-" * 20)

    # --- Test Case 4: Payment Inquiry ---
    ...
    # --- Test Case 5: Follow-up in Payment Stage ---
