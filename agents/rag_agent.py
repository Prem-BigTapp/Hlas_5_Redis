...
class RAGAgent:
    def answer_query(self, query: str, chat_history: list, product: str = None):
        ...
        """
        # --- Normalize product (accept Enum or str) ---
        # Convert to canonical TRAVEL/MAID/CAR to match KB metadata
        if hasattr(product, "value"):                          # [prev: not handled]
            product = product.value                            # [prev: N/A]
        product_norm = (product or "").strip().upper()         # [prev: product used as-is]
        if product_norm not in {"TRAVEL", "MAID", "CAR"}:
            return "Please specify which insurance product you're asking about (Travel, Maid or Car)."  # unchanged text

        try:
            logger.info(f"🔍 RAG QUERY - Product: {product_norm}, Query: '{query}'")  # [prev: product]
            ...
            response = collection.query.hybrid(
                ...
                filters=Filter.by_property("product_name").equal(product_norm),  # [prev: equal(product)]
            ...
            # (be defensive if a property is missing)
            chunks = []                                       # [prev: joined directly from properties['content']]
            for obj in response.objects:                      # [prev: context_str = \"\\n---\\n\".join([...]) ]
                props = getattr(obj, "properties", {}) or {}
                content = props.get("content") or ""
                questions = props.get("questions") or ""
                # Prefer content; append questions only if it adds info
                if content:
...
"""),
            ...
            formatted_response = self._add_guidance(llm_response.content, product_norm, query)  # [prev: product]

            logger.info(f"✅ RAG Response Generated: {len(formatted_response)} characters")
            return formatted_response

        except Exception as e:
            logger.error(f"❌ Error querying {product_norm} collection: {e}")  # [prev: {product}]
            return f"I'm having trouble accessing the {product_norm.title()} insurance information right now. Could you please rephrase your question?"  # [prev: {product}]
    ...
    def _add_guidance(self, response_text: str, product: str, original_query: str):
        ...
        if not is_plan_comparison:
            ...
            if is_purchase_related:
                ...
                response_text += f"\n\n*Ready to purchase?*\nType 'proceed to payment' to continue."  # [prev: 'proceed with purchase']
            else:
                # Add standard guidance for general queries
                response_text += (
                    f"\n\nNeed more details? Just ask.\n"
                    f"Ready to purchase? Type 'buy {product.lower()} insurance'."
                )  # unchanged behavior
